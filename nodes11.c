...
node_top:
	...
	for (i=old_number_best;i<=(*counter);i++) {/*swap on input tree_reps*/
		...
		for (j=1+ntaxa;j<sub_taxa-1+ntaxa;j++) {
			n_left=remainder_local(n_nodes,ntaxa,sub_taxa,j-ntaxa,is_descendent,is_descendent2);
...
int remainder_local(nodes,ntaxa,sub_taxa,node_taken,is_descendent,is_descendent2)
